...
for category, num_attributes in CATEGORY_ATTRIBUTES.items():
    ...
    if not (os.path.exists(efficientnet_model_path) and os.path.exists(encoder_path)):
    ...
    for i in range(0, len(image_paths), BATCH_SIZE):
        ...
        decoded_preds = decode_predictions(efficientnet_preds, current_encoders, num_attributes)
...
submission.fillna('missing_value', inplace=True)
